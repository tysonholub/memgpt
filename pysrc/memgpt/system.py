...
def package_summarize_message(summary, summary_length, hidden_message_count, total_message_count, timestamp=None):
    context_message = (
        f"Note: prior messages ({hidden_message_count} of {total_message_count} total messages) have been hidden from"
        " view due to conversation memory constraints.\n"
...
def package_summarize_message_no_summary(hidden_message_count, timestamp=None, message=None):
    ...
    context_message = (
        ...
        if message
            ...
            f"Note: {hidden_message_count} prior messages with the user have been hidden from view due to conversation"
            " memory constraints. Older messages are stored in Recall Memory and can be viewed using functions."
        )
    )
    packaged_message = {
        "type": "system_alert",
