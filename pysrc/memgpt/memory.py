...
from .utils import cosine_similarity, count_tokens, get_local_time, printd
...
class CoreMemory:
    ...
    def __init__(
        self, persona=None, human=None, persona_char_limit=None, human_char_limit=None, archival_memory_exists=True
    ):
    ...
    def edit_persona(self, new_persona):
        if self.persona_char_limit and len(new_persona) > self.persona_char_limit:
            error_msg = (
                f"Edit failed: Exceeds {self.persona_char_limit} character limit (requested {len(new_persona)})."
            )
            if self.archival_memory_exists:
                error_msg = (
                    f"{error_msg} Consider summarizing existing core memories in 'persona' and/or moving lower"
                    " priority content to archival memory to free up space in core memory, then trying again."
                )
    ...
    def edit_human(self, new_human):
        if self.human_char_limit and len(new_human) > self.human_char_limit:
            ...
            if self.archival_memory_exists:
                error_msg = (
                    f"{error_msg} Consider summarizing existing core memories in 'human' and/or moving lower priority"
...
async def summarize_messages(
    ...
    if summary_input_tkns > MESSAGE_SUMMARY_WARNING_TOKENS:
        ...
        summary_input = str(
            [await summarize_messages(model, message_sequence_to_summarize[:cutoff])]
...
class DummyArchivalMemory(ArchivalMemory):
    ...
    def __init__(self, archival_memory_database=None):
        self._archive = (
            [] if archival_memory_database is None else archival_memory_database
        )  # consists of {'content': str} dicts
    ...
    async def search(self, query_string, count=None, start=None):
        ...
        printd(
            f"archive_memory.search (text-based): search for query '{query_string}' returned the following results"
            f" (limit 5):\n{[matches[start:count]]}"
        )
...
class DummyArchivalMemoryWithEmbeddings(DummyArchivalMemory):
    ...
    async def search(self, query_string, count=None, start=None):
        ...
        printd(
            f"archive_memory.search (vector-based): search for query '{query_string}' returned the following results"
            " (limit 5) and"
...
class DummyArchivalMemoryWithFaiss(DummyArchivalMemory):
    ...
    async def search(self, query_string, count=None, start=None):
        ...
        printd(
            f"archive_memory.search (vector-based): search for query '{query_string}' returned the following results"
            f" ({start}--{start+5}/{len(search_result)}) and"
...
class DummyRecallMemory(RecallMemory):
    ...
    async def text_search(self, query_string, count=None, start=None):
        ...
        printd(
            f"recall_memory.text_search: searching for {query_string} (c={count}, s={start}) in"
            f" {len(self._message_logs)} total messages"
        )
        matches = [
            d
            for d in message_pool
            if d["message"]["content"] is not None and query_string.lower() in d["message"]["content"].lower()
        ]
    ...
    async def date_search(self, start_date, end_date, count=None, start=None):
        ...
        matches = [
            ...
            for d in message_pool
            if start_date_dt
            <= datetime.datetime.strptime(self._extract_date_from_timestamp(d["timestamp"]), "%Y-%m-%d")
            <= end_date_dt
        ]
    ...
    async def text_search(self, query_string, count=None, start=None):
        ...
        for d in message_pool:
            ...
            if self.only_use_preloaded_embeddings:
                ...
                self.embeddings[message_str] = await async_get_embedding_with_backoff(
                    message_str, model=self.embedding_model
                )
                message_pool_filtered.append(d)
        ...
        query_embedding = await async_get_embedding_with_backoff(query_string, model=self.embedding_model)
        similarity_scores = [
            cosine_similarity(self.embeddings[d["message"]["content"]], query_embedding) for d in message_pool_filtered
        ]
        ...
        printd(
            f"recall_memory.text_search (vector-based): search for query '{query_string}' returned the following"
            " results (limit 5) and"
