...
from .memory import CoreMemory as Memory
from .memory import summarize_messages
...
from .utils import count_tokens, get_local_time, parse_json, printd, united_diff
...
def construct_system_with_memory(system, memory, memory_edit_timestamp, archival_memory=None, recall_memory=None):
    full_system_message = "\n".join(
        [
            ...
            (
                f"{len(recall_memory) if recall_memory else 0} previous messages between you and the user are stored"
                " in recall memory (use functions to access them)"
            ),
            (
                f"{len(archival_memory) if archival_memory else 0} total memories you created are stored in archival"
                " memory (use functions to access them)"
            ),
...
class AgentAsync:
    ...
    async def handle_ai_response(self, response_message):
        ...
        if response_message.get("function_call"):
            ...
            if not (isinstance(heartbeat_request, bool) or heartbeat_request is None):
                printd(
                    f"Warning: 'request_heartbeat' arg parsed was not a bool or None, type={type(heartbeat_request)},"
                    f" value={heartbeat_request}"
                )
    ...
    async def step(
        self, user_message, first_message=False, first_message_retry_limit=FIRST_MESSAGE_ATTEMPTS, skip_verify=False
    ):
        ...
        try:
            ...
            if not skip_verify and (first_message or self.messages_total == self.messages_total_init):
                ...
                while True:
                    response = await get_ai_reply_async(
                        model=self.model, message_sequence=input_message_sequence, functions=self.functions
                    )
                    if self.verify_first_message_correctness(
                        response, require_monologue=self.first_message_verify_mono
                    ):
                        break
            ...
            if current_total_tokens > MESSAGE_SUMMARY_WARNING_TOKENS:
                printd(
                    f"WARNING: last response total_tokens ({current_total_tokens}) > {MESSAGE_SUMMARY_WARNING_TOKENS}"
                )
            ...
            if "maximum context length" in str(e):
                ...
                printd(
                    "step() failed with openai.InvalidRequestError, but didn't recognize the error message:"
                    f" '{str(e)}'"
                )
                raise e
    ...
    async def summarize_messages_inplace(self, cutoff=None):
        ...
        printd(
            f"Attempting to summarize {len(message_sequence_to_summarize)} messages [1:{cutoff}] of"
            f" {len(self.messages)}"
        )
    ...
    async def recall_memory_search(self, query, count=5, page=0):
        results, total = await self.persistence_manager.recall_memory.text_search(
        ...
        if len(results) == 0:
            ...
            results_formatted = [
                f"timestamp: {d['timestamp']}, {d['message']['role']} - {d['message']['content']}" for d in results
            ]
    ...
    async def recall_memory_search_date(self, start_date, end_date, count=5, page=0):
        results, total = await self.persistence_manager.recall_memory.date_search(
